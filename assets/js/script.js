...
let bonusFoodArray = [];
let pendingBonusBitcoins = 0;

// Init function called by mining.js
window.initGameWithBonus = (bonusCount) => {
  ...
  if (startPopup) {
    ...
    // Hide background menu buttons to clean up UI
    const menuBtns = document.getElementById("menu-buttons-container");
    if (menuBtns) menuBtns.classList.add("hidden");
  }
...
const snakeColor = "#F7931A"; // Bitcoin Orange
...
const animate = () => {
  ...
  scoreBoard.draw();
  food.draw();
  bonusFoodArray.forEach(bf => bf.draw());
  snake.draw();
...
class Food {
  ...
  draw() {
    ctx.save();
    ctx.moveTo(this.x + tile * 0.55, this.y + tile * 0.1);
    ctx.lineTo(this.x + tile * 0.25, this.y + tile * 0.6);
    ctx.lineTo(this.x + tile * 0.5, this.y + tile * 0.6);
    ctx.lineTo(this.x + tile * 0.3, this.y + tile * 0.9);
    ctx.lineTo(this.x + tile * 0.75, this.y + tile * 0.4);
    ctx.lineTo(this.x + tile * 0.5, this.y + tile * 0.4);
    ctx.closePath();
...
class BonusFood extends Food {
  ...
  draw() {
    ...
    // Draw Bitcoin 'B'
    ctx.fillStyle = "#FFF";
    ctx.font = `bold ${tile * 0.7}px serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    // Adjust y slightly for visual alignment
    ctx.fillText("₿", this.x + tile / 2, this.y + tile * 0.55);

    ctx.restore();
  }
}

const spawnBonusFood = (count) => {
  bonusFoodArray = [];
  for (let i = 0; i < count; i++) {
    bonusFoodArray.push(new BonusFood());
  }
  // Simple collision avoidance with snake or other bonus food could be added here
};
...
let game = {
  ...
  ateFood: undefined,
  ateBonusFood: undefined, // Add this
  lastMove: undefined,
  state: undefined,
  ...
  checkAteFood() {
    ...
    // Check Bonus Food
    this.ateBonusFood = false;
    for (let i = 0; i < bonusFoodArray.length; i++) {
      if (snake.newHead.x === bonusFoodArray[i].x && snake.newHead.y === bonusFoodArray[i].y) {
        this.ateBonusFood = true;
        bonusFoodArray.splice(i, 1);
        break; // Only eat one per turn
      }
    }
  },
  ...
  update() {
    ...
    if (this.collisionDetected) {
      ...
      scoreBoard.update();
      scoreBoard.update();
      // scoreBoard.print(); // Disable old scoreboard text
      // this.changeState("GAMEOVER"); // Disable old gameover state change handling that shows menu
      game.stop(); // Stop the game loop

      // Trigger Gamestr Game Over
      if (window.GAMESTR) {
        window.GAMESTR.handleGameOver(scoreBoard.currentScore);
      } else {
      ...
      // Do NOT respawn or increment gamespeed
      // eats food:
...
document
  .querySelectorAll("#scores-button")
  .forEach((el) =>
